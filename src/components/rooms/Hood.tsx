 ...
 * grabación no hay cifras, y se dice.
 */
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-5 lg:gap-14">
      ...
      <div className="lg:col-span-2">
        <h3 className="label">{dict.run.title}</h3>
        ...
        <dl className="marker mt-4">
          ...
          <div className="marker-row">
            ...
            <dd>{run.response?.model ?? "—"}</dd>
          </div>
          <div className="marker-row">
            <dt>{dict.run.input}</dt>
            <dd className="tnum">{usage ? dict.run.tokens.replace("{n}", fmt.format(usage.input_tokens)) : "—"}</dd>
          </div>
          <div className="marker-row">
            <dt>{dict.run.reasoning}</dt>
            <dd className="tnum">{usage ? dict.run.tokens.replace("{n}", fmt.format(reasoning)) : "—"}</dd>
          </div>
          <div className="marker-row">
            <dt>{dict.run.output}</dt>
            <dd className="tnum">{usage ? dict.run.tokens.replace("{n}", fmt.format(usage.output_tokens - reasoning)) : "—"}</dd>
          </div>
          <div className="marker-row">
            <dt>{dict.run.time}</dt>
            <dd className="tnum">{ms === null ? "—" : `${(ms / 1000).toFixed(2)} s`}</dd>
          </div>
          <div className="marker-row">
