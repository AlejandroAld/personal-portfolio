...
export function generateStaticParams() {
  return LOCALES.flatMap((lang) =>
    NODES.flatMap((n) => (SUBNODES[n.id] ?? []).map((s) => ({ lang, node: n.slug[lang], sub: s.slug }))),
  );
}
...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; node: string; sub: string }> }): Promise<Metadata> {
  ...
  const s = n && subBySlug(n.id, sub);
  if (!n || !s) return {};
  const dict = getDictionary(lang);
  const role = dict.experience.roles[s.ref];
  return {
    title: `${role?.headline ?? s.slug} — ${dict.meta.title}`,
    alternates: { canonical: absoluteUrl(pathFor(lang, n.id, s.slug)) },
  };
}
...
export default async function SubPage({ params }: { params: Promise<{ lang: string; node: string; sub: string }> }) {
  ...
  const s = n && subBySlug(n.id, sub);
  if (!n || !s) notFound();
  return <Site lang={lang} node={n.id} sub={s.slug} />;
}
